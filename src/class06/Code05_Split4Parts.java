...
例如：arr={3,2,4,1,4,9,5,10,1,2,2} 返回true，三个切点分别为2,5，7，切出的四个子数组为[3,2] [1,4] [5] [1,2,2]

解法：
- 先生成一个前缀和数组，记录到map中，key：前缀和， value：位置
- 枚举每个位置，然后计算前缀和的值，第一刀是i位置，0 - i-1的累加和事x，那么第二刀的位置必须是，前缀和等于2x+a
**/
package class06;
