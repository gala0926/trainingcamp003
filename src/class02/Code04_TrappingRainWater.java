/**
给定一个数组arr，已知其中所有的值都是非负数，将这个数组看做一个容器，请返回容器能装多少水
比如，arr = {3, 1, 2, 5, 2, 4},根据值画出的直方图就是容器形状，容器可以装下5格水
再比如，arr = {4, 5, 1, 3, 2}, 容器可以装下2格水

解法：求每个一个i位置上可以存多少水

min(max左，max右）- arr[i] > 0 ? min(max左，max右）: 0
**/
package class02;
